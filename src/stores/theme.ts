...
export const useThemeStore = defineStore("theme", {
  state: () => {
    const primaryTenant = (JSON.parse(localStorage.getItem("tenant") || "{}")?.primary_color as string) || "";

    const modifiedColors = { ...tokens["colors"] }; 
    if (primaryTenant) {
      modifiedColors.light.primary = primaryTenant;
      modifiedColors.dark.primary = primaryTenant; 
    }
    ...
    return {
      ...
      logoTenant: (JSON.parse(localStorage.getItem("tenant") || "{}")?.logo as string) || "",
      primaryTenant, 
    };
  },
  actions: {
    getLogoTenant(): string {
      return this.logoTenant;
    },
    getPrimaryTenant(): string {
      return this.primaryTenant;
    },
    updatePrimaryColor(newPrimaryColor: string): void {
      this.primaryTenant = newPrimaryColor;
      this.colors.light.primary = newPrimaryColor;
      this.colors.dark.primary = newPrimaryColor;
    },
    ...
    updateVars() {
        ...
        if (_get(this, key) && key in this.responsive.override) {
          _set(
            ...
            window.innerWidth <= this.responsive.breakpoint
              ? this.responsive.override[
                  key as keyof typeof this.responsive.override
                ].mobile
              : this.responsive.override[
                  key as keyof typeof this.responsive.override
                ].desk
      ...
      if (this.sidebar.autoClose) {
        if (
          ...
          window.innerWidth <= this.sidebar.autoCloseBreakpoint
        ) {
          this.sidebar.collapsed = true;
        }
      }
  ...
  getters: {
    ...
    style(state): { [key: string]: string } {
      ...
      const { success005, warning005, error005, info005 } =
        state.colors[state.themeName];
      ...
      const { divider005, divider010, divider020, divider030 } =
        this.dividerColors;
      ...
      const { secondary1, secondary2, secondary3, secondary4 } =
    ...
    isBoxed(state): boolean {
      return false; // state.boxed.enabled
    },
    isRTL(state): boolean {
      return state.rtl;
    },
    ...
    isToolbarBoxed(state): boolean {
      return false; // state.boxed.toolbar && state.boxed.enabled
    },
  },
  persist: {
